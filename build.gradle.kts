...
// Substitutes these tokens in source with the values from `gradle.properties`.
blossom {
...
version = mod_version
// Matches the source package, me.mint.tntfusetimer.
group = "me.mint"
...
loom {
    ...
    // No tweak class and no mixins: EntityTNTPrimed.fuse is public in 1.8.9, so this mod
    // reads it directly and needs no bytecode manipulation at all.
}
...
dependencies {
    // Deliberately empty.
    //
    // This mod ships zero third-party runtime dependencies:
    //  - no OneConfig (its stage0 loader downloads ~17MB at startup; we make no network requests)
    //  - no Mixin (EntityTNTPrimed.fuse is public in 1.8.9)
    //  - no DevAuth (no unofficial authentication anywhere in this project)
    //
    // Config storage and the settings screen both come from Forge itself
    ...
    jar {
        ...
        if (platform.isLegacyForge) {
            manifest.attributes += mapOf(
                "ModSide" to "CLIENT", // Client-only mod.
                "ForceLoadAsMod" to true // Make Forge load this jar as a mod.
            )
        }
        dependsOn(shadowJar)
        archiveClassifier.set("")
...
// Gradle and Loom themselves need Java 17, but the 1.8.9 client must run on Java 8:
// LaunchWrapper's Launch.<init> casts the system classloader to URLClassLoader, which
// only holds on 8. Without this, runClient dies instantly with a ClassCastException.
